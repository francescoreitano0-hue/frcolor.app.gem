...
async function openMod(id) {
    ...
    if(id === 'irina') {
        try {
            ...
            applyZoom(1); document.getElementById('zoom-range').value = 1;
...
function applyZoom(val) {
    const v = document.getElementById('v'), cv = document.getElementById('c-view');
    v.style.transform = `scale(${val})`;
    cv.style.transform = `scale(${val})`;
}

// LETTURA COLORE (Tocco e Centro)
...
function processColor(r, g, b) {
    ...
    let m = RAL_DB.map(c => ({...c, d: Math.sqrt((r-c.r)**2+(g-c.g)**2+(b-c.b)**2)})).sort((a,b)=>a.d-b.d)[0];
    document.getElementById('color-preview').style.backgroundColor = hex;
    document.getElementById('out-text').innerHTML = `
        <b style="font-size:18px;">RAL ${m.ral}</b> - ${m.n}<br>
...
function addCantiere() {
    ...
    state.cantieri.push({ id: Date.now(), nome: n, note: "", materiali: "", foto: [] });
...
function renderCantieri() {
    document.getElementById('c-list').innerHTML = state.cantieri.map(c => `
        <div class="folder">
            <div style="display:flex; justify-content:space-between;"><b>📁 ${c.nome}</b> <button onclick="delC(${c.id})" style="color:red; border:none; background:none;">Elimina</button></div>
            <textarea placeholder="Note cantiere..." onchange="updC(${c.id},'note',this.value)">${c.note}</textarea>
            <textarea placeholder="Materiali utilizzati..." onchange="updC(${c.id},'materiali',this.value)">${c.materiali}</textarea>
...
function delC(id) { if(confirm("Eliminare cantiere?")) { state.cantieri = state.cantieri.filter(x=>x.id!==id); save(); renderCantieri(); } }
...
function addF(id, cam) {
    ...
    i.onchange = e => {
        const r = new FileReader(); r.onload = () => { 
            state.cantieri.find(x => x.id === id).foto.push(r.result); 
            save(); alert("Foto salvata nel cantiere!");
...
function updStock(mode) {
    ...
    if(!n || isNaN(q)) return;
    let i = state.mag[currentTab].find(x => x.nome.toLowerCase() === n.toLowerCase());
    if(i) {
        i.qta = (mode==='IN' ? i.qta + q : Math.max(0, i.qta - q));
        i.note = nt;
    } else if(mode==='IN') {
        state.mag[currentTab].push({ nome: n, qta: q, note: nt });
    }
    save(); renderMag();
    document.getElementById('m-nome').value = ""; document.getElementById('m-qta').value = ""; document.getElementById('m-note').value = "";
}

function renderMag() {
    document.getElementById('m-list').innerHTML = state.mag[currentTab].map((i, idx) => `
        <div class="item-row">
            <span><b>${i.nome}</b><br><small>${i.note || ''}</small></span>
            <div style="text-align:right;"><b>${i.qta}</b><br><button onclick="delM(${idx})" style="color:red; border:none; background:none; font-size:10px;">Rimuovi</button></div>
        </div>
    `).join('');
}

function delM(idx) { state.mag[currentTab].splice(idx,1); save(); renderMag(); }
