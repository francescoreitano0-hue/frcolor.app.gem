...
let state = JSON.parse(localStorage.getItem('FR_PRO_FINAL_V2')) || { cantieri: [], mag: [], att: [] };
let stream = null;
...
function addCantiere() {
    ...
    state.cantieri.push({ id: Date.now(), nome: n, materiali: "", note: "", foto: [], docs: [] });
...
function renderCantieri() {
    document.getElementById('c-list').innerHTML = state.cantieri.map(c => `
        <div class="c-card">
            ...
            <label style="font-size:11px; font-weight:bold; margin-top:10px; display:block;">📸 CARTELLA FOTO</label>
            <div class="foto-grid">${(c.foto || []).map(f => `<img src="${f}" class="thumb">`).join('')}</div>
            <button class="btn-in" onclick="addF(${c.id},'foto',true)" style="width:100%; margin-bottom:10px;">+ FOTO</button>

            <label style="font-size:11px; font-weight:bold; display:block;">📄 CARTELLA DOCUMENTI</label>
            <div style="font-size:12px; margin:5px 0;">${(c.docs || []).length} file salvati</div>
            <button class="btn-out" onclick="addF(${c.id},'docs',false)" style="width:100%;">+ DOCUMENTO</button>
        </div>`).reverse().join('');
}

function updC(id, f, v) { state.cantieri.find(x => x.id === id)[f] = v; save(); }

function addF(id, tipo, cam) {
    const i = document.createElement('input'); i.type = 'file'; if(cam) i.capture = 'environment';
    i.onchange = e => {
        const r = new FileReader();
        r.onload = () => { 
            const cant = state.cantieri.find(x => x.id === id);
            if(!cant[tipo]) cant[tipo] = [];
            cant[tipo].push(r.result); 
            save(); renderCantieri(); 
        };
...
function addAttrezzo() {
    const n = document.getElementById('a-nome').value, nt = document.getElementById('a-note').value;
    if(!n) return;
    if(!state.att) state.att = [];
    state.att.push({ nome: n, note: nt });
    save(); renderMag();
    document.getElementById('a-nome').value = ""; document.getElementById('a-note').value = "";
}

function renderMag() {
    let html = '<b>PRODOTTI</b>';
    html += state.mag.map(i => `<div class="c-card"><b>${i.nome}</b> - Q.tà: ${i.qta}<br><small>${i.note || ''}</small></div>`).join('');
    html += '<b style="display:block; margin-top:20px;">ATTREZZATURE</b>';
    if(!state.att) state.att = [];
    html += state.att.map(a => `<div class="c-card"><b>${a.nome}</b><br><small>${a.note || ''}</small></div>`).join('');
    document.getElementById('m-list').innerHTML = html;
}

function save() { localStorage.setItem('FR_PRO_FINAL_V2', JSON.stringify(state)); }
