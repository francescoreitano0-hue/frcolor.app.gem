...
function checkLogin() {
    ...
    } else { document.getElementById('login-err').innerText = "Errata"; }
}
...
function takeReading() {
    ...
    const p = cv.getContext('2d').getImageData(cv.width/2, cv.height/2, 1, 1).data;
    showCol(p[0], p[1], p[2]);
}
    ...
    document.getElementById('c-list').innerHTML = state.cantieri.map(c => `
        <div class="folder">
            ...
            <div class="btn-grid-row">
                ...
                <button class="btn-out" onclick="addF(${c.id}, false)">🖼️ GALLERY</button>
            </div>
        </div>
    `).reverse().join('');
}
...
function addF(id, cam) {
    ...
    i.onchange = e => {
        const r = new FileReader(); r.onload = () => { state.cantieri.find(x => x.id === id).foto.push(r.result); save(); renderCantieri(); };
...
function updStock(m) {
    ...
    if(!n || isNaN(q)) return;
    let i = state.mag[currentTab].find(x => x.nome === n);
    if(i) i.qta = (m==='IN'?i.qta+q:Math.max(0,i.qta-q));
    else if(m==='IN') state.mag[currentTab].push({ nome: n, qta: q });
    save(); renderMag();
}

function renderMag() {
    document.getElementById('m-list').innerHTML = state.mag[currentTab].map(i => `
        <div class="item-row"><span>${i.nome}</span><b>${i.qta}</b></div>
    `).join('');
}
