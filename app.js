...
function renderCantieri() {
    document.getElementById('c-list').innerHTML = state.cantieri.map(c => `
        <div class="folder">
            <b>📁 ${c.nome}</b>
            <textarea placeholder="Note Generali..." onchange="updC(${c.id},'note',this.value)">${c.note}</textarea>
            <textarea placeholder="Materiali (vai a capo per elenco)..." onchange="updC(${c.id},'materiali',this.value)" style="height:120px;">${c.materiali}</textarea>
            <div class="gal">${c.foto.map(f => `<img src="${f}">`).join('')}</div>
            <div class="btn-grid-row">
                <button class="btn-in" onclick="addFoto(${c.id}, true)">📷 SCATTA</button>
                <button class="btn-out" style="background:#64748b" onclick="addFoto(${c.id}, false)">🖼️ GALLERIA</button>
            </div>
        </div>
    `).reverse().join('');
}
...
function updStock(mode) {
    ...
    else if(mode==='IN') state.mag[currentTab].push({ nome: n, qta: q, note: nt });
    save(); renderMag();
}
...
function renderMag() {
    document.getElementById('m-list').innerHTML = state.mag[currentTab].map(i => `
        <div class="item-row">
            <span><b>${i.nome}</b><br><small style="color:#666">${i.note || 'Senza note'}</small></span>
            <b style="font-size:18px">${i.qta}</b>
        </div>
    `).join('');
}
...
async function openMod(id) {
    ...
    if(id === 'irina') {
        stream = await navigator.mediaDevices.getUserMedia({ video: { facingMode: "environment", width: { ideal: 3840 }, height: { ideal: 2160 } } });
...
function analyzeImg(input) {
    ...
    reader.onload = (e) => {
        const img = new Image();
        img.onload = () => {
            const cv = document.getElementById('c-view'); cv.width = img.width; cv.height = img.height;
            const ctx = cv.getContext('2d'); ctx.drawImage(img, 0, 0);
            document.getElementById('v').style.display = 'none'; cv.style.display = 'block';
...
function showCol(r, g, b) {
    ...
    document.getElementById('out-text').innerHTML = `<b>RAL ${m.ral}</b> (${m.n})<br><small>HEX: ${hex} | RGB: ${r},${g},${b}</small><br><small>SCR: ${(r/255).toFixed(2)}, ${(g/255).toFixed(2)}</small>`;
}

function save() { localStorage.setItem('FR_PRO_FINAL_V1', JSON.stringify(state)); }
