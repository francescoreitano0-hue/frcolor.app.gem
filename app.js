...
function renderCantieri() {
    document.getElementById('c-list').innerHTML = state.cantieri.map(c => `
        <div class="c-card" style="border: 2px solid #1e3a5f; padding:15px; border-radius:15px; margin-top:15px; background:#fff;">
            ...
            <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 10px;">
                ...
                <div style="background:#f8f9fa; padding:10px; border-radius:10px; border:1px solid #ccc;">
                    ...
                    <div style="font-size:11px; margin-bottom:10px;">File: ${(c.docs || []).length}</div>
...
function addFile(id, tipo, camera) {
    const i = document.createElement('input'); i.type = 'file'; i.accept = 'image/*';
    if(camera) i.capture = 'environment';
    i.onchange = e => {
        const r = new FileReader();
        r.onload = () => { 
            let c = state.cantieri.find(x => x.id === id);
            if(!c[tipo]) c[tipo] = [];
            c[tipo].push(r.result); 
            save(); renderCantieri(); 
        };
...
function renderMag() {
    document.getElementById('m-list').innerHTML = state.mag.map(i => `
        <div style="padding:10px; border-bottom:1px solid #eee;"><b>${i.nome}</b> - Q.tà: ${i.qta}</div>`).join('');
    document.getElementById('a-list').innerHTML = state.att.map(i => `
        <div style="padding:10px; border-bottom:1px solid #eee; background:#f9f9f9;"><b>${i.nome}</b> - Q.tà: ${i.qta}<br><small>${i.note || ''}</small></div>`).join('');
}
