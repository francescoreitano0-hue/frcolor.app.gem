...
let state = JSON.parse(localStorage.getItem('FR_PRO_V6')) || { cantieri: [], mag: [], att: [] };
let stream = null;
...
// --- IRINA (ZOOM E PUNTAMENTO INVARIATI) ---
...
function renderCantieri() {
    document.getElementById('c-list').innerHTML = state.cantieri.map(c => `
        <div class="c-card" style="border:1px solid #ddd; padding:10px; margin-top:10px; border-radius:10px; background:#fff;">
            <b>📁 ${c.nome}</b>
            <textarea placeholder="Materiali..." onchange="updC(${c.id},'materiali',this.value)" style="width:100%; height:40px; margin:5px 0;">${c.materiali || ''}</textarea>
            <div style="display:flex; gap:10px;">
                <div style="flex:1; background:#f0f0f0; padding:5px; border-radius:5px;">
                    <small>📸 FOTO (${(c.foto||[]).length})</small>
                    <button onclick="addFile(${c.id},'foto')" style="width:100%;">+</button>
                </div>
                <div style="flex:1; background:#f0f0f0; padding:5px; border-radius:5px;">
                    <small>📄 DOCS (${(c.docs||[]).length})</small>
...
// --- MAGAZZINO (PRODOTTI + ATTREZZATURE) ---
...
function save() { localStorage.setItem('FR_PRO_V6', JSON.stringify(state)); }
