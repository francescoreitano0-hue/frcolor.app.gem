...
let state = JSON.parse(localStorage.getItem('FR_PRO_V5')) || { cantieri: [], mag: [], att: [] };
let stream = null;
...
// --- IRINA SPETTROMETRO (CONGELATO + FIX LETTURA) ---
...
function takeReading() {
    ...
    // Legge il pixel centrale del canvas (puntamento fisso)
...
// --- CANTIERI (CONGELATO + CARTELLE) ---
function addCantiere() {
    const n = document.getElementById('c-nome').value; if(!n) return;
    state.cantieri.push({ id: Date.now(), nome: n, materiali: "", foto: [], docs: [] });
...
function renderCantieri() {
    document.getElementById('c-list').innerHTML = state.cantieri.map(c => `
        <div class="c-card" style="border:1px solid #ddd; padding:10px; margin-top:10px; border-radius:10px;">
            <b>📁 ${c.nome}</b>
            <textarea placeholder="Materiali..." onchange="updC(${c.id},'materiali',this.value)" style="width:100%; height:40px; margin-top:5px;">${c.materiali || ''}</textarea>
            <div style="display:flex; gap:10px; margin-top:10px;">
                <div style="flex:1; background:#f9f9f9; padding:5px; border-radius:5px;">
                    <small>📸 FOTO</small>
                    <div class="foto-grid">${(c.foto || []).map(f => `<img src="${f}" class="thumb" style="width:40px; height:40px;">`).join('')}</div>
                    <button onclick="addFile(${c.id},'foto')" style="width:100%;">+</button>
                </div>
                <div style="flex:1; background:#f9f9f9; padding:5px; border-radius:5px;">
                    <small>📄 DOCS</small>
                    <div style="font-size:10px;">${(c.docs || []).length} file</div>
                    <button onclick="addFile(${c.id},'docs')" style="width:100%;">+</button>
                </div>
            </div>
        </div>`).reverse().join('');
}

function addFile(id, tipo) {
    const i = document.createElement('input'); i.type = 'file';
    i.onchange = e => {
        const r = new FileReader();
        r.onload = () => { 
            state.cantieri.find(x => x.id === id)[tipo].push(r.result); 
            save(); renderCantieri(); 
        };
...
function updStock(mode) {
    ...
    else if(mode==='IN') state.mag.push({ nome: n, qta: q });
    save(); renderMag();
}

function addAttrezzo() {
    const n = document.getElementById('a-nome').value; if(!n) return;
    if(!state.att) state.att = [];
    state.att.push(n); save(); renderMag();
    document.getElementById('a-nome').value = "";
}

function renderMag() {
    let h = '<b>Prodotti:</b><br>';
    h += state.mag.map(i => `${i.nome} (Q.tà: ${i.qta})<br>`).join('');
    h += '<br><b>Attrezzature:</b><br>';
    h += (state.att || []).map(a => `- ${a}<br>`).join('');
    document.getElementById('m-list').innerHTML = h;
}

function save() { localStorage.setItem('FR_PRO_V5', JSON.stringify(state)); }
