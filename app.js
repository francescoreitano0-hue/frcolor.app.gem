...
let state = JSON.parse(localStorage.getItem('FR_PRO_V3')) || { cantieri: [], mag: { mat: [], att: [] } };
let stream = null; let currentTab = 'mat';
...
function applyZoom(val) {
    ...
    if(v.style.display !== 'none') v.style.transform = `scale(${val})`;
    else cv.style.transform = `translate(${translateX}px, ${translateY}px) scale(${val})`;
}

// GESTIONE TRASCINAMENTO FOTO
...
cv.addEventListener('touchmove', (e) => {
    ...
    cv.style.transform = `translate(${translateX}px, ${translateY}px) scale(${zoom})`;
}, {passive: false});
...
function takeReading() {
    ...
    if (v.style.display !== 'none') {
        ...
        ctx.drawImage(v, 0, 0);
        showCol(ctx.getImageData(cv.width/2, cv.height/2, 1, 1).data);
    } else {
        const rect = document.getElementById('cam-container').getBoundingClientRect();
        const photoRect = cv.getBoundingClientRect();
        const x = ( (rect.left + rect.width/2) - photoRect.left ) * (cv.width / photoRect.width);
        const y = ( (rect.top + rect.height/2) - photoRect.top ) * (cv.height / photoRect.height);
        showCol(ctx.getImageData(x, y, 1, 1).data);
    }
}
...
function addF(id, cam) {
    ...
    i.onchange = e => {
        const r = new FileReader(); r.onload = () => { 
            state.cantieri.find(x => x.id === id).foto.push(r.result); save(); renderCantieri(); 
...
function save() { localStorage.setItem('FR_PRO_V3', JSON.stringify(state)); }
