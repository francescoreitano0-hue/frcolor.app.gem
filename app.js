// DATABASE RAL CLASSIC COMPLETO (213 CODICI)
const RAL_DB = [
...
let state = JSON.parse(localStorage.getItem('FR_PRO_SYSTEM_2025_V2')) || { cantieri: [], mag: { mat: [], att: [] } };
let stream = null;
let currentTab = 'mat';
...
function applyZoom(val) {
    document.getElementById('v').style.transform = `scale(${val})`;
    document.getElementById('c-view').style.transform = `scale(${val})`;
}
...
function showCol(r, g, b) {
    ...
    // Parametri Schermo
    const scrR = (r/255).toFixed(2);
    const scrG = (g/255).toFixed(2);
    const scrB = (b/255).toFixed(2);

    document.getElementById('color-preview').style.backgroundColor = hex;
    document.getElementById('out-text').innerHTML = `
        <div class="data-box">RAL: ${match.ral}</div>
        <div class="data-box">HEX: ${hex}</div>
        <div class="data-box">RGB: ${r},${g},${b}</div>
        <div class="data-box">SCR: ${scrR},${scrG},${scrB}</div>
...
function addCantiere() {
    ...
    state.cantieri.push({ id: Date.now(), nome: n, note: "", foto: [] });
...
function renderCantieri() {
    document.getElementById('c-list').innerHTML = state.cantieri.map(c => `
        <div class="c-card">
            <div style="display:flex; justify-content:space-between;"><b>📁 ${c.nome}</b><button onclick="delC(${c.id})" style="border:none;background:none;color:red;">Elimina</button></div>
...
function updStock(m) {
    ...
    if(i) { i.qta = (m==='IN'?i.qta+q:Math.max(0,i.qta-q)); i.note = nt; }
    else if(m==='IN') state.mag[currentTab].push({ nome: n, qta: q, note: nt });
    save(); renderMag();
...
// UTILS
function save() { localStorage.setItem('FR_PRO_SYSTEM_2025_V2', JSON.stringify(state)); }
